...
void QXmppRtcpPacket::write(QDataStream &stream) const
{
    ...
    if (d->type == Goodbye) {
        ...
        Q_FOREACH (quint32 ssrc, d->goodbyeSsrcs)
            s << ssrc;
        ...
        Q_FOREACH (const QXmppRtcpReceiverReport &report, d->receiverReports)
            report.d->write(s);
    } else if (d->type == SourceDescription) {
        count = d->sourceDescriptions.size();
        Q_FOREACH (const QXmppRtcpSourceDescription &desc, d->sourceDescriptions)
            desc.d->write(s);
    } else {
        count = d->count;
        payload = d->payload;
